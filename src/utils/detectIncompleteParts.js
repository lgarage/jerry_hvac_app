/**
 * Detect incomplete part specifications
 *
 ...
 *
 * This module checks part strings for required specifications and returns
 * structured data about what's missing.
 */

/**
 * Check if a part string has complete specifications
 *
 * @param {string} partString - Part description (e.g., "filters", "24x24x2 filters", "AA batteries")
 * @returns {Object} Detection result with completeness status and missing details
 */
export function detectIncompletePart(partString) {
  if (!partString) {
    return {
      isComplete: false,
      category: 'unknown',
  ...
  return {
    isComplete: normalized.split(' ').length >= 2, // At least 2 words
    category: 'generic',
    missingDetails: [],
    prompt: null,
    confidence: 0.5,
 ...
 * Filters MUST have size (e.g., 24x24x2, 20x25x1)
 */
function detectFilter(normalized, original) {
  ...
  // Check for size pattern: 16x20x1, 24x24x2, 20x25x4, etc.
  const sizePattern = /\b(\d{1,2})\s*[xX×]\s*(\d{1,2})\s*[xX×]\s*(\d{1,2})\b/;
  const hasSize = sizePattern.test(original);

  if (hasSize) {
    const match = original.match(sizePattern);
    const size = `${match[1]}x${match[2]}x${match[3]}`;
 ...
 * Batteries MUST have type (AA, AAA, 9V, etc.)
 */
 ...
 * Contactors MUST have poles AND voltage
 */
 ...
 * Capacitors MUST have MFD (microfarads) AND voltage
 */
 ...
 * Refrigerant MUST have type (R-410A, R-22) AND quantity (in pounds or ounces)
 */
function detectRefrigerant(normalized, original) {
  ...
  // Check for refrigerant type (R-410A, R-22, R-134A, R-404A, R-407C, R-32)
  const typePattern = /\br[-\s]?(410a|22|134a|404a|407c|32)\b/i;
  const hasType = typePattern.test(normalized);

  // Check for quantity (4 lbs, 2 pounds, 16 oz, 1.5 lbs)
 ...
 * Belts MUST have size or part number
 */
function detectBelt(normalized, original) {
  ...
  // Check for belt size (5/8 x 54, A54, B42, 4L540)
 ...
 * Motors MUST have HP, voltage, and speed
 */
function detectMotor(normalized, original) {
  ...
  // Check for HP (1/2 HP, 1 HP, 3/4 HP)
  ...
  // Check for speed (1075 RPM, 825 RPM)
 ...
 * Thermostats should specify type and stages
 */
function detectThermostat(normalized, original) {
  ...
  // Check for type (programmable, non-programmable, digital, mechanical)
  const typePattern = /\b(programmable|digital|mechanical|smart|wifi)\b/i;
  const hasType = typePattern.test(normalized);

  // Check for stages (2H/1C, single stage, multi-stage)
  ...
  if (hasType || hasStages) {
    // Either type or stages makes it reasonably complete
    return {
      isComplete: true,
      category: 'thermostat',
      missingDetails: [],
 ...
 * @returns {Array<Object>} Detection results for each part
 */
export function detectIncompletePartsBatch(parts) {
 ...
 *
 * @param {Array<Object>} detectionResults - Results from detectIncompletePartsBatch
 * @returns {Array<string>} Array of clarification questions
 */
export function getClarificationQuestions(detectionResults) {
 ...
 *
 * @param {Array<Object>} detectionResults - Results from detectIncompletePartsBatch
 * @returns {boolean} True if any parts need clarification
 */
export function hasIncompleteParts(detectionResults) {
